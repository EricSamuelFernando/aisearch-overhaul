resource "aws_cloudfront_origin_access_control" "s3_oac" {
  name                              = "${var.project_name}-s3-oac-${var.env}-terraform"
...
resource "aws_cloudfront_origin_access_control" "lambda_oac" {
  name                              = "${var.project_name}-lambda-oac-${var.env}-terraform"
...
data "aws_cloudfront_response_headers_policy" "lambda_response_header_policy" {
  name = "SnaphomzAuthPolicy"
}
...
resource "aws_cloudfront_distribution" "distribution" {
  ...
  default_cache_behavior {
    ...
    response_headers_policy_id = data.aws_cloudfront_response_headers_policy.lambda_response_header_policy.id
  }
  ...
  tags = {
    "Name"        = "${var.project_name}-frontend-cdn-${var.env}-terraform"
    "Environment" = var.env
  }
}
