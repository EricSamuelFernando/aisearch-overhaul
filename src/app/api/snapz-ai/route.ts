...
Return ONLY:
{
  ...
  try {
    ...
    searchQuery = parsed.searchQuery || parsed.query || '';
  } catch (e) {
    console.error('[snapz-ai] Failed to parse OpenAI response:', e, raw);
    return [];
  }

  if (!searchQuery) {
    console.error('[snapz-ai] No search query generated by AI');
    return [];
  }
  ...
  const searchResponse = await fetch(searchUrl, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ query: searchQuery, user: userId }),
  });
  ...
  const rawResults =
    searchData?.search_results?.records ||
  ...
  console.log('[snapz-ai] Extraction Debug:', {
    hasSearchResults: !!searchData?.search_results,
    hasProperties: !!searchData?.properties,
    rawResultsType: typeof rawResults,
    isRawResultsArray: Array.isArray(rawResults),
    rawResultsLength: Array.isArray(rawResults) ? rawResults.length : 'N/A',
  });
  ...
  console.log('[snapz-ai] Found', filtered.length, 'results after filtering (originally', results.length, ')');
...
export async function POST(request: Request) {
  try {
    ...
    if (body.phase === 'recommend') {
      const { snapProperties, analysis, answers, dismissedListingIds, likedListingIds, userId } = body as RecommendPhaseBody;
      ...
      const properties = await generateRecommendations(apiKey, snapProperties, analysis, answers, dismissedListingIds, likedListingIds, userId);
