...
// const AGENT_COPILOT_INTRO =
//   "I'm Snaphomz AI - the assistant inside the Snaphomz platform designed to help real estate agents work smarter.";

// const AGENT_COPILOT_SYSTEM_PROMPT = [
//   "SYSTEM ROLE: SNAPHOMZ AGENT AI",
//   "Identity: You are Snaphomz AI, an assistant inside the Snaphomz platform for licensed U.S. real estate agents.",
//   "Do not claim you are the company itself. Never say 'Snaphomz is my name.'",
//   `When introducing yourself, use this line exactly: "${AGENT_COPILOT_INTRO}"`,
//   "Primary purpose: Act as an agent copilot to help win clients, close transactions, analyze listings, respond to buyers/sellers, and improve workflow.",
//   "Communication style: professional, confident, concise, industry-aware, and practical.",
//   "Use U.S. real-estate context: MLS, escrow, title, contingencies, appraisal/inspection, closing disclosures, earnest money, Fair Housing, NAR, broker supervision, and state-level compliance.",
//   "Never give legal or tax advice. Say: 'Check with your broker or local regulations.'",
//   "Do not provide discriminatory guidance or violate Fair Housing principles.",
//   "When asked about Snaphomz: Snaphomz is an end-to-end platform; you are a feature within Snaphomz.",
// ].join("\n");

// const applyAgentPersonaGuardrails = (
//   text: string,
//   userQuery: string,
//   enabled: boolean
// ) => {
...
//   let normalized = text;
//   normalized = normalized.replace(/snaphomz is my name\.?/gi, AGENT_COPILOT_INTRO);

//   const asksForIntro = /\b(who are you|what are you|introduce yourself|your role)\b/i.test(userQuery);
//   const hasCorrectIntro = /i(?:'| a)m\s+snaphomz ai/i.test(normalized);
//   if (asksForIntro && !hasCorrectIntro) {
//     normalized = `${AGENT_COPILOT_INTRO}\n\n${normalized}`;
//   }
...
//   const shouldUseAgentCopilotPersona =
//     assistantMode === 'agent' ||
...
//     { id: '4', text: 'What is the monthly payment on a $240,000 loan at 6% for 30 years?' },
//   ];
//   const buyerSuggestions = suggestions;
//   const sellerSuggestions = suggestions;
...
//           question: queryToSearch,
//           session_id: sessionId,
//           system_prompt: shouldUseAgentCopilotPersona ? AGENT_COPILOT_SYSTEM_PROMPT : undefined,
//           assistant_mode: shouldUseAgentCopilotPersona ? 'agent' : undefined,
//         };
//         if (selectedPropertyId !== null) {
//           questionPayload.selected_property_id =
//             selectedPropPayload?.propertyId ??
...
//           data.response ||
//           "I couldn't find a response for that question.";
//         const normalizedAiText = applyAgentPersonaGuardrails(
//           aiText,
//           queryToSearch,
//           shouldUseAgentCopilotPersona
//         );
...
//       const isCardsOnly = responseMode === "cards_only";
//       const hasAiText =
//         typeof normalizedAiText === "string" && normalizedAiText.trim().length > 0;
...
//           // Priority to forecast if explicitly asked?
//           // Or combine?
//           if (!finalContent) finalContent = normalizedAiText || "Here is the forecast.";
//         } else if (mappedProps.length > 0) {
//           finalContent = (!isCardsOnly && hasAiText)
//             ? normalizedAiText
//             : `I found ${mappedProps.length} homes that match your criteria.`;
//         } else {
//           // Try to extract schools from text if no homes found
//           const extraction = extractSchoolsFromText(normalizedAiText || "");
//           if (extraction && extraction.schools.length > 0) {
//             extractedSchools = extraction.schools;
//             extractedAddress = extraction.address || undefined;
//             finalContent = `I found ${extraction.schools.length} schools near ${extractedAddress || 'the location'}.`;
//           } else {
//             finalContent = normalizedAiText || "I couldn't find any properties matching that search right now.";
//           }
//         }
...
//           id: aiMsgId,
//           role: 'assistant',
//           content: normalizedAiText || "I couldn't find any properties matching that search right now.",
//           relatedProperties: [],
//           relatedQuestions: relatedQuestions,
//           intent: data.intent
//         };
...
//     setShowAddressSuggestions(false);
//     setAddressSuggestions([]);
//     handleSearchSubmit(suggestion.address);
//   };
...
//     setShowLocationSuggestions(false);
//     setLocationSuggestions([]);
//     handleSearchSubmit(suggestion.description);
//   };
...
//   const isHomeLanding = pathname?.includes('/home');
//   const currentSuggestions = isSeller ? sellerSuggestions : buyerSuggestions;
//   const isSuggestionsOpen = !searchTerm && isInputFocused;
//   const isCardExpanded = !disableAutoExpand && (isExpanded || isSuggestionsOpen);
//   const shouldAnimateWidth = isHomeLanding && !disableAutoExpand;
//   const isWidened = shouldAnimateWidth && isSuggestionsOpen;
//   const expandedMaxWidth = 1120;
//   const collapsedWidthPercent = 88;
//   const openRadius = 24;
//   const closedRadius = 12;
//   const containerClasses = cn(
//     "bg-white shadow-xl shadow-black/5 mx-auto bg-clip-padding relative overflow-visible w-full",
//     showOutline &&
...
//     onSearchStateChange(isSuggestionsOpen, searchTerm);
//   }, [isSuggestionsOpen, onSearchStateChange]);
