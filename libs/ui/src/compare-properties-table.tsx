...
type ListingData = {
    ...
    media?: {
        photosList?: ListingMediaPhoto[] | null;
    } | null;
};
...
export interface ComparePropertiesTableProps {
    slotProperties: Array<CompareProperty | null>;
    activeSlotIndex: number;
    recentlyFilledSlotIndex?: number | null;
    onSelectSlot: (slotIndex: number) => void;
    onRemoveFromSlot: (slotIndex: number) => void;
    onClose: () => void;
    getStateFromZip?: (zipCode?: string) => string | null | undefined;
    imageLoader?: (params: ImageLoaderProps) => string;
}
...
const formatUsdCurrency = (value: number | null): string | null => {
...
const getBestIndex = (values: Array<number | null>, mode: 'highest' | 'lowest'): number => {
    ...
    const best =
        mode === 'highest'
            ? validValues.reduce((a, b) => (b.value! > a.value! ? b : a))
            : validValues.reduce((a, b) => (b.value! < a.value! ? b : a));
...
const Badge = ({ className = 'ml-1.5' }: { className?: string }) => (
    <span
        className={`${className} inline-flex h-[18px] w-[18px] flex-shrink-0 items-center justify-center rounded-full bg-[#FF8700]`}
    >
        <Star className="h-[9px] w-[9px] fill-white text-white stroke-[2.4]" />
    </span>
);

const ComparePropertiesTable = ({
    slotProperties,
    activeSlotIndex,
    recentlyFilledSlotIndex = null,
    onSelectSlot,
    onRemoveFromSlot,
    onClose,
    getStateFromZip,
    imageLoader,
}: ComparePropertiesTableProps) => {
    ...
    const prices = normalizedSlots.map((property) => toPositiveNumber(property?.price));
    const beds = normalizedSlots.map((property) => toPositiveNumber(property?.bedRooms));
    const baths = normalizedSlots.map((property) => toPositiveNumber(property?.bathRooms));
    const sqfts = normalizedSlots.map((property) =>
        toPositiveNumber(property?.livingArea ?? property?.sqft)
    );
    const pricePerSqft = normalizedSlots.map((property) => {
        const price = toPositiveNumber(property?.price);
        const area = toPositiveNumber(property?.livingArea ?? property?.sqft);
    ...
    const bestPricePerSqftIdx = getBestIndex(pricePerSqft, 'lowest');
    ...
    const sections: CompareSection[] = [
        {
            ...
            rows: [
                {
                    label: 'Bedrooms',
                    values: beds.map((value) => (value ? String(value) : null)),
                    bestIdx: bestBedsIdx,
                },
                {
                    label: 'Bathrooms',
                    values: baths.map((value) => (value ? String(value) : null)),
                    bestIdx: bestBathsIdx,
                },
                {
                    label: 'Living Area',
                ...
                {
                    label: 'Status',
                    values: normalizedSlots.map(
                        (property) => property?.listing?.standardStatus || property?.status || null
                    ),
                    bestIdx: -1,
                },
            ],
        },
        {
            ...
            rows: [
                {
                    label: 'Address',
                    values: normalizedSlots.map((property) => property?.address || null),
                    bestIdx: -1,
                },
                {
                    label: 'City',
                    values: normalizedSlots.map((property) => property?.city || null),
                    bestIdx: -1,
                },
                {
                    label: 'State / Zip',
                    values: normalizedSlots.map((property) => {
                        const state = getStateFromZip?.(property?.zipCode ?? undefined);
                ...
                <div>
                    ...
                    {selectedCount < 2 && (
                        <p className="mt-1 text-xs font-medium text-orange-500">
                            Select at least 2 properties to compare data.
                        </p>
                    )}
                </div>
                <button
                ...
                <div className="flex border-b border-gray-100">
                    ...
                    <div className="flex flex-1 gap-2 overflow-x-auto p-2">
                        {normalizedSlots.map((property, index) => {
                            ...
                            const displayState = getStateFromZip?.(property?.zipCode ?? undefined);
                            const image =
                                property?.listing?.media?.photosList?.[0]?.lowRes || property?.image;
                            ...
                            return (
                                <div
                                    ...
                                    className={`min-w-[220px] flex-1 max-w-[360px] cursor-pointer overflow-hidden rounded-2xl border border-gray-100 shadow-sm transition-all duration-300 hover:shadow-md ${
                                        ...
                                        isRecentlyFilled
                                            ? 'scale-[1.015] shadow-[0_10px_28px_rgba(255,135,0,0.22)]'
                                            : ''
                                ...
                                >
                                    ...
                                    <div className={`px-4 py-3 text-center ${isBestPrice ? 'bg-[#FF8700]' : 'bg-gray-900'}`}>
                                        <p className="text-base font-bold leading-tight text-white">
                                            {formatUsdCurrency(toPositiveNumber(property?.price)) || '-'}
                                        </p>
                                    </div>
                ...
                {sections.map((section) => (
                    <div key={section.title}>
                        ...
                        {section.rows.map((row) => (
                            <div
                                key={row.label}
                                className="flex border-b border-gray-100 transition-colors hover:bg-orange-50/30 last:border-0"
                            >
