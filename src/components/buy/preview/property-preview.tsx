    ...
    <>
      ...
      {loading ? (
        ...
        <div className='grid grid-cols-1 gap-3 sm:gap-4 md:grid-cols-12 md:gap-7 h-auto md:h-[28rem] transition-all duration-300 ease-in-out px-4 sm:px-6 md:px-0'>
          ...
          />
          <div className="relative md:fixed right-0 top-20  sm:mt-4 md:mt-[136px] mr-0 md:mr-4 transition-all duration-300 ease-in-out w-full md:w-auto">
            <ListingAgentCard
          ...
          <div className='md:col-span-9 md:-mt-4 md:h-[28rem]'>
            <div className='mt-2 flex flex-col sm:flex-row w-full justify-between items-start sm:items-center gap-3 sm:gap-4 md:gap-0'>
              <div className="space-y-2 w-full sm:w-auto">
                ...
                <p className='truncate text-clip text-xl font-bold sm:text-base text-gray-600 leading-5 sm:leading-6'>{`${transformData.prop?.address?.unparsedAddress || propertyDatas?.property_detail?.data?.propertyInfo?.address?.address || "N/A"}, ${transformData.prop?.address?.city || propertyDatas?.property_detail?.data?.propertyInfo?.address?.city || "N/A"}, ${transformData.prop?.address?.stateOrProvince || propertyDatas?.property_detail?.data?.propertyInfo?.address?.stateOrProvince || "N/A"}, ${transformData.prop?.address?.zipCode || propertyDatas?.property_detail?.data?.propertyInfo?.address?.zip || "N/A"}` || transformData.prop?.listingAgent?.fullName || "N/A"}</p>
              </div>
