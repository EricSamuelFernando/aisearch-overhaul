  ...
  return (
    <>
      <div className='my-8 flex h-full flex-col gap-6 px-4 sm:px-6'>
        {/* Mobile: Horizontal steps with icons only */}
        ...
        <div className='md:hidden grid grid-cols-1 gap-6 pt-4'>
          ...
          {currentStep === 1 ? <div className='flex w-full h-auto flex-col justify-start gap-6 rounded-2xl bg-white px-4 py-6'>
            <h2 className='text-lg font-bold'>Choose your means</h2>
        ...
        {/* Desktop: Vertical sidebar with text */}
        <div className='hidden md:grid md:grid-cols-4 gap-x-6'>
          <div className='col-span-1'>
            <div className='space-y-1 border-r border-[#DBE2EE] pt-8'>
              {stepList.map((step, index) => (
                <CurrentGuideStep
      ...
      <div className='mt-auto w-full flex flex-col sm:flex-row items-center sm:justify-between gap-3 px-0 pb-5 md:px-5'>
        <div className='flex flex-row flex-nowrap items-center gap-3 w-full sm:w-auto'>
          <Link
            href=""
            onClick={(e) => {
              e.preventDefault();
            ...
            className='inline-flex h-8 items-center justify-center rounded-full border-2 border-black bg-transparent px-3 sm:px-8 py-2 text-center text-black'
          >
            Back
          </Link>
          <Link
            ...
            className='px-3 sm:px-8 py-2 font-bold text-ocOrange'
          >
            Cancel
          </Link>
        </div>

        <div className="flex flex-nowrap items-center gap-5 w-full sm:w-auto justify-end">
          <TooltipProvider>
            {isDisabled ? (
              <Tooltip>
                <TooltipTrigger asChild>
                  <div>
                    <Button
                      disabled
                      className="flex items-center justify-center rounded-full bg-black px-4 sm:px-8 py-2 text-center text-white w-full sm:w-auto"
                    >
                      {isRedirecting
                        ? "Redirecting..."
                        : isLoading
...
const CurrentGuideStep = ({ step, isActive, onClick, isMobile = false }: CurrentStepProp) => {
  const { icon, title, desc, key } = step;
  const Icon = icon ?? null;
  const Dot = isActive ? Icons.CircleDotFilled : Icons.CircleDot;
  ...
  if (isMobile) {
    ...
    return (
      <div
        key={key}
        onClick={onClick}
        className='flex cursor-pointer flex-col items-center gap-1 flex-shrink-0'
      >
        <div
          className={cn(
            'flex items-center justify-center rounded-full p-2',
            isActive ? 'bg-ocOrange' : 'bg-white border border-gray-200',
          )}
        >
          <Icon className='h-6 w-6' />
        </div>
        {isActive && <div className='h-1 w-1 rounded-full bg-ocOrange' />}
      </div>
    );
  }

  // Desktop: Full layout with text
  return (
    <div
      key={key}
      onClick={onClick}
