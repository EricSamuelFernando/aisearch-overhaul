...
function PropertyDetailLayout() {
  ...
  return (
    <div>
      {(!loading && !isLoading) ? (
        <div className='grid h-full min-h-screen md:grid-cols-5 gap-x-6'>
          <div className='items-between container flex h-full flex-col md:col-span-3'>
            <div className='mb-10 flex flex-1 flex-col md:flex-row items-start justify-between gap-x-4'>
              <PropertyOverview
              ...
              <div className='flex h-full flex-auto  flex-col items-center justify-between mt-4 mx-auto'>
                {(agent?.[0]?.is_accepted !== "rejected" && agent?.[0]?.agent?.email) ? (
                  <AgentCard agent={agent?.[0]} property={propertyData} />
                ) : (
