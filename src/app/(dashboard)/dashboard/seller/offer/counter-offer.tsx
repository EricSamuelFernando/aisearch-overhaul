...
import { useFetchPropertyCounterOffers, useGetPropertyOffersByProperty, usePropertyServiceAPI } from '@/hooks/api/agent/useAgentProperty';
import { useQueryClient } from '@tanstack/react-query';

const TransactionCounterOffers = () => {
  const [view, setView] = useState('grid');
  const queryClient = useQueryClient();
  const [showAddComment, setShowAddComment] = useState(false);
  const claimedProperty = useSelector((state: any) => state?.property?.claimProperty);
  const params = useSearchParams();
  const {finalizeOffer} = usePropertyServiceAPI()
  const id = params?.get('id');
  const {
    getClaimedPropertyOffersAPI,
  } = SellerAPIs();
  const { data: propertyOffers, isLoading: offerLoading, error,refetch } = useFetchPropertyCounterOffers(claimedProperty?.id?.toString(), claimedProperty?.listingid?.toString());
  ...
  useEffect(() => {
    // Trigger the refetch whenever the page loads or the claimedProperty changes
    if (claimedProperty?.id) {
  ...
  console.log(claimedProperty, propertyOffers)
...
export default TransactionCounterOffers;
